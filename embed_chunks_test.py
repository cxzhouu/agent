...
def build_embedding_text(record: Dict[str, Any]) -> str:
    """
    为人物知识块构造更适合检索的 embedding 文本。

    设计目标：
    1. 保留正文 + 结构信息
    2. 强化人物名、块类型、标签、章节路径
    3. 对问答类 chunk 显式拼接 question / answer
    4. 对课堂场景类 chunk 保留标题和路径，方便后续课堂提问命中
    """
...
def process_one_doc(
    doc_id: str,
    project_root: Path,
    chunk_root: str = "data/chunk",
    output_root: str = "data/embedding",
    batch_size: int = BATCH_SIZE,
    jina_api_key: Optional[str] = None,
) -> Dict[str, Any]:
    """
    处理单个人物知识库的 embedding 生成。
    """
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="人物知识库 chunk -> embedding 通用脚本")

    parser.add_argument(
        "--doc_id",
        type=str,
        default=None,
        help="单个人物文档 ID，例如 li_dazhao"
    )
    parser.add_argument(
        "--chunk_root",
        type=str,
        default="data/chunk",
        help="chunk 根目录，默认 data/chunk"
    )
    parser.add_argument(
        "--output_root",
        type=str,
        default="data/embedding",
        help="embedding 输出根目录，默认 data/embedding"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
def main():
    ...
    if args.batch_default_heroes:
        results = process_default_heroes(
            project_root=project_root,
    ...
    raise ValueError("请传入 --doc_id 或使用 --batch_default_heroes")
